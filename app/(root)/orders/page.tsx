...
const Orders = async () => {
  ...
  return (
    <div className="px-10 py-5 max-sm:px-3">
      <p className="text-heading3-bold my-10">Seus Pedidos</p>
      {!orders ||
        (orders.length === 0 && (
          <p className="text-body-bold my-5">Você ainda não tem pedidos.</p>
        ))}
        ...
        {orders?.map((order: OrderType) => (
          <div className="flex flex-col gap-8 p-4 hover:bg-grey-1">
            <div className="flex gap-20 max-md:flex-col max-md:gap-3">
              <p className="text-base-bold">ID Pedido: {order._id}</p>
